...
const PossibleActions = [
  ActionValues.AddVideo,
  ActionValues.UpdateVideo,
  ActionValues.AddSpeaker,
  ActionValues.UpdateSpeaker,
  ActionValues.UpdateTimeline,
];
...
export default function Upload() {
  ...
  const exportSpeakersXLSX = async () => {
    ...
    let speakerz = [
      ["id", "name", "designation", "photoURL", "profileLine", "social"],
    ];
    speakersArr.forEach((speaker) => {
      ...
      Object.keys(speaker.social).forEach((ele) => {
        social +=
          social == ""
            ? speaker.social[ele]
            : breakElement + speaker.social[ele];
      });
  ...
  const processAddVideos = async (object, callback) => {
    ...
    await asyncForEach(object, async (element) => {
      try {
        ...
        const id = await VideoManager.addVideo(
          element.title,
          element.description,
          element.videoUrl,
          element.thumnailUrl,
    ...
    if (callback) {
      callback("All the videos have been added to the database.");
    }
  };
  ...
  const processUpdateVideos = async (object, callback) => {
    ...
    await asyncForEach(object, async (element) => {
      try {
        ...
        let speakerArr = element.speakers.split(breakElement);
        let processedSpeakerArr = speakerArr.map((tt) => tt.trim());
        const id = await VideoManager.completelyUpdateVideo(
          element.id,
          element.title,
          element.description,
          element.videoUrl,
    ...
    await exportVideosXLSX();
    if (callback) {
      callback("The videos have been updated in the database.");
    }
  };
  ...
  const processAddSpeakers = async (object, callback) => {
    ...
    await asyncForEach(object, async (element) => {
      try {
        ...
        const id = await SpeakerManager.makeSpeaker(
          element.name,
          element.designation,
          processArr,
          element.photoURL,
    ...
    if (callback) {
      callback("All the speakers have been added to the database.");
    }
  };
  ...
  const processUpdateSpeakers = async (object, callback) => {
    ...
    await asyncForEach(object, async (element) => {
      try {
        ...
        const id = await SpeakerManager.completelyUpdateSpeaker(
          element.id,
          element.name,
          element.designation,
          processArr,
    ...
    await exportSpeakersXLSX();
    if (callback) {
      callback("The Speakers have been updated in the database.");
    }
  };
    ...
    >
      <form>
        ...
        <button
          type="button"
          id="addUsers"
          onClick={() => exportVideosXLSX()}
          disabled={inUse}
        >
          Download All Videos XLSX
        </button>
        <br></br>
        <br></br>
        <button
          type="button"
          id="addUsers"
          onClick={() => exportSpeakersXLSX()}
          disabled={inUse}
        >
          Download All Speakers XLSX
        </button>
      </form>
    </div>
  );
