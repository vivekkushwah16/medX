...
export function PollResult(props) {
  ...
  return (
    <div className="communityBox__question">
      <h3 className="communityBox__title">{`Q${
        currentIndex + 1
      }. ${question}`}</h3>
  ...
  return (
    <div class="communityBox__question">
      ...
      <form onSubmit={handleSubmitButton}>
        <ul class="communityBox__options">
          {options.map((option) => (
            <li key={`option-${option.id}`}>
              <label key={`checkbox-${option.id}`} class="custom-checkbox3">
                {option.value}
                <input
                  type="radio"
                  name={option.id}
        ...
        {showError && (
          <>
            <span style={{ color: "red", marginBottom: "0.75rem" }}>
        ...
        <button
          type="submit"
          class="btn btn-secondary mg-t5"
          disabled={checkIfAlreadyAnswered === null ? false : true}
        >{`${checkIfAlreadyAnswered === null ? "Submit" : "Answered"}`}</button>
      </form>
    </div>
  );
}
