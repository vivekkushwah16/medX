...
export default function AgendaNavBar(props) {
    ...
    const { className, currentDate, dates, handleClick, containerClass, forceAgendaVisibleMobile, stickyOnScroll } = props
                    ...
                    <ul className="navigarion-bar__menu" style={forceAgendaVisibleMobile ? { borderBottom: '0', justifyContent: 'unset' } : {}}>
                        {
                            dates &&
                            dates.map(date => (
                                <li key={`agednaNav-${className}-${date}`} className={`${currentDate === date ? 'active' : ''}`} onClick={(event) => handleClick(date, event)}><button className={`btn btn-sm btn-secondary ${currentDate === date ? 'active' : ''}`}>{date}</button></li>
                            ))
                        }
                    </ul>
                </div>
