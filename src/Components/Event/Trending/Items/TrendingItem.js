    ...
    >
            ...
            } */}
      {
        <div
          ...
          style={{
            ...
            backgroundColor: "rgba(1,81,138,0.6)",
          }}
        >
          <div className="tint"></div>
        </div>
      ...
      <div className="maincardBox__card-body">
        <div className="text-block">
          <h4 className="mg-b15 maincardBox__card-title">{data.title}</h4>
          <ReadMore
        ...
        >
          {data.type === TRENDING_ITEM_TYPE.VIDEO && (
            <button
              ...
              onClick={() => {
                handleClickAnalytic();
                showMediaModal(MediaModalType.Videos, data.link);
              }}
            >
              Play <i className="icon-lock"></i>
            </button>
          )}
          {data.type === TRENDING_ITEM_TYPE.IMAGE && (
            <button
              ...
              onClick={() => {
                handleClickAnalytic();
                showMediaModal(MediaModalType.Image, data.link);
              }}
            >
  ...
  return (
    <div className="trendingBox__item">
      ...
      <div className="trendingBox__item-text">
        ...
        <ReadMore
          className={"trendingBox__item-desc"}
          description={data.description}
        />
      </div>
      <div className="trendingBox__item-action">
        {data.type === TRENDING_ITEM_TYPE.VIDEO && (
          <button
            className="btn btn-outline btn-locked"
            disabled={data.disabled}
            onClick={() => showMediaModal(MediaModalType.Videos, data.link)}
          >
            Play <i className="icon-lock"></i>
          </button>
        )}
        {data.type === TRENDING_ITEM_TYPE.IMAGE && (
          <button
            className="btn btn-outline btn-locked"
            disabled={data.disabled}
            onClick={() => showMediaModal(MediaModalType.Image, data.link)}
          >
            Open <i className="icon-lock"></i>
          </button>
        )}
        {data.type === TRENDING_ITEM_TYPE.PDF && (
          <button
            className="btn btn-outline btn-locked"
            disabled={data.disabled}
            onClick={() => showMediaModal(MediaModalType.PDF, data.link)}
          >
            Open <i className="icon-lock"></i>
          </button>
        )}
